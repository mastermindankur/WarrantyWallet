...
export const dailyReminderJob = functions.scheduler.onSchedule('every day 09:00', async (event) => {
    ...
    try {
        ...
        const userWarrantiesMap = new Map<string, { expiring: Warranty[], expired: Warranty[], future: Warranty[] }>();

        const processSnapshot = (snapshot: any, type: 'expiring' | 'expired' | 'future') => {
...
async function sendReminderEmail({ userEmail, expiringWarranties, expiredWarranties, futureWarranties }: SendReminderEmailParams) {
    if (!resend || !fromEmail) {
